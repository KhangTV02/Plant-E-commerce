...
const UploadProduct = ({ onClose, fetchData }) => {
  ...
  const handleOnChange = (e) => {
    ...
    // ) {
    //   toast.error("Vui lòng nhập giá trước khi nhập giá khuyến mãi");
    //   return;
    // }
  };
  ...
  const handleOnBlur = (e) => {
    ...
    if (name === "sellingPrice") {
      if (!data.price || parseFloat(data.price) <= 0) {
        toast.error("Vui lòng nhập giá trước khi nhập giá khuyến mãi");
      } else if (numValue >= parseFloat(data.price)) {
        toast.error("Giá khuyến mãi phải nhỏ hơn giá gốc");
      }
    }

    if (name === "stock" && numValue <= 0) {
  ...
  const handleUploadProduct = async (e) => {
    ...
    const uploadImageCloudinary = await uploadImage(file);
     ...
     if (uploadImageCloudinary?.url) {
      // Trích xuất đặc trưng ảnh sau khi upload
    

      setData((prev) => ({
        ...prev,
        plantImage: [...prev.plantImage, uploadImageCloudinary.url],       
      }));
    }
  };
  ...
  //   const numValue = parseFloat(value) || 0; // Chuyển thành số, nếu rỗng thì là 0

  //   if (name === "stock" || name === "price") {
  //     return numValue > 0; // Số lượng và giá phải lớn hơn 0
  //   }

  //   if (name === "sellingPrice") {
  //     return numValue >= 0 && numValue < parseFloat(data.price || 0);
  //     // Giá khuyến mãi phải >= 0 và nhỏ hơn giá gốc
  //   }
  //   return true;
  // };
  ...
  return (
    <div className="mt-16 fixed w-full  h-full bg-slate-200 bg-opacity-35 top-0 left-0 right-0 bottom-0 flex justify-center items-center">
      <div className="bg-white p-4 rounded w-full max-w-2xl h-full max-h-[80%] overflow-hidden">
        <div className="flex justify-between items-center pb-3">
          <h2 className="font-bold text-lg">Tạo sản phẩm</h2>
          <div
